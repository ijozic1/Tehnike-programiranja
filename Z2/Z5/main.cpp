...
int main() {
    ...
    std::cin >> velicina;
    try {
        std::vector<double> a(velicina);
        std::cout << "\nElementi prve sekvence: ";
        for (double &x : a) std::cin >> x;
        std::deque<double> b(velicina);
        std::cout << "\nElementi druge sekvence: ";
        for (double &t : b) std::cin >> t;
        auto tablica = KreirajTablicuSabiranja(a.begin(), a.end(), b.begin());
        std::cout << "\nTablica sabiranja: " << std::endl;
        for (int i = 0; i < velicina; i++) {
            for (int j = 0; j < i + 1; j++) std::cout << tablica[i][j] << " ";
            std::cout << std::endl;
        }
        delete[] tablica[0]; delete[] tablica;
    } 
    catch (std::logic_error &izuzetak) {
